...
begin
  ...
  dmGenData.Query2.SQL.Text :=
                  ...
                  dmGenData.Query2.ParamByName('idInd').AsInteger := lIdInd;
                  dmGenData.Query2.ParamByName('sex').AsString := lSex;
                  dmGenData.Query2.Open;
                  result := not  dmGenData.Query2.EOF;
                  dmGenData.Query2.Close;
end;
...
begin
    fraPhrase1.TypePhrase := dmGenData.GetTypePhrase(PtrInt(cbxEvType.Items.Objects[cbxEvType.ItemIndex]));
    ...
    if fraPhrase1.isDefault then
      begin
        ...
        fraPhrase1.FullPhrase :=
            DecodePhrase(frmStemmaMainForm.iID, fraPhrase1.WType, fraPhrase1.TypePhrase, 'N', edtIdName.Value);
      end
    else
...
begin
    ...
    if FEditMode <> eNET_EditExisting then
      ...
      begin
        ...
        //dmGenData.
        dmGenData.getNameData(edtIdName.Value,lidInd,lidType,temp,lMemo,lPhrase,lDate,lSortDate,lPref);
        ...
        fraPhrase1.FullPhrase := DecodePhrase(frmStemmaMainForm.iID, 'PRINCIPAL', fraPhrase1.Text, 'N', edtIdName.Value);
        fraDate1.Date := lDate;
...
begin
    ...
    if (edtPrefered.Text = '1') and
        ((frmStemmaMainForm.iID <> lidInd) and not
        (lidInd = 0) and not (edtPrefered2.Text = '1')) then
        edtPrefered.Text := '0'// Si on déplace un nom primaire d'individu, le nom devient secondaire.
    ;
    ...
    if frmStemmaMainForm.actWinExplorer.Checked then
        if lidName <> 0 then
            ...
            frmExplorer.AppendIndex(i2, i1, nom, lidName, lidInd, edtPrefered.Text = '1');
    if lidName = 0 then
        lidName := dmGenData.GetLastIDOfTable('N');
    ...
    case FEditMode of
        eNET_EditExisting, eNET_AddSpouse:
          begin
            ...
            lDestType := 'E';
            dmGenData.CopyCitationOfIndByType(lidInd, lSourceType, lidInd_Dest, lDestType);
...
begin
    if frmEditName.ActiveControl.Name = 'tblNames' then
      begin
        found := False;
    ...
    if frmEditName.ActiveControl.Name = 'edtDateForSorting' then
      begin
        found := False;
        for j := frmStemmaMainForm.DataHist.Row to frmStemmaMainForm.DataHist.RowCount - 1 do
    ...
    if frmEditName.ActiveControl.Name = 'edtDateForPresentation' then
      begin
        found := False;
        for j := frmStemmaMainForm.DataHist.Row to frmStemmaMainForm.DataHist.RowCount - 1 do
    ...
    if frmEditName.ActiveControl.Name = 'edtIdInd' then
      begin
        found := False;
        for j := frmStemmaMainForm.DataHist.Row to frmStemmaMainForm.DataHist.RowCount - 1 do
    ...
    if frmEditName.ActiveControl.Name = 'edtPrefered' then
      begin
        found := False;
        for j := frmStemmaMainForm.DataHist.Row to frmStemmaMainForm.DataHist.RowCount - 1 do
    ...
    if frmEditName.ActiveControl.Name = 'edtMemoText' then
      begin
        found := False;
        for j := frmStemmaMainForm.DataHist.Row to frmStemmaMainForm.DataHist.RowCount - 1 do
