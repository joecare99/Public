...
type
  ...
  private
    ...
    FRelID: integer;
    procedure DecodeFullName(sFullName: string; out sSuffix, sFamilyName,
      sGivenName, sTitle: string);
...
begin
  fraPhrase1.TypePhrase:=dmGenData.GetTypePhrase(PtrInt(Y.Items.Objects[Y.ItemIndex]));
  // Todo: Update fraPhrase1 Data
  if fraPhrase1.isDefault then
  begin
    fraPhrase1.WType:='PRINCIPAL';
    fraPhrase1.idLink:=No.Value;

    fraPhrase1.FullPhrase :=
      DecodePhrase(frmStemmaMainForm.iID,fraPhrase1.WType, fraPhrase1.TypePhrase,'N' , No.Value);
  end
  else
...
//  TableauNoms.Columns.c:=3;
  ...
  begin
    ...
    fraPhrase1.Text := dmGenData.Query1.Fields[5].AsString;
    fraPhrase1.TypePhrase :=dmGenData.GetTypePhrase(PtrInt(Y.Items.Objects[Y.ItemIndex]));
    ...
    fraPhrase1.FullPhrase := DecodePhrase(frmStemmaMainForm.iID, 'PRINCIPAL', fraPhrase1.Text, 'N', No.Value);
    fraDate1.Date := dmGenData.Query1.Fields[6].AsString;
    fraDate1.SortDate := dmGenData.Query1.Fields[7].AsString;
