...
USES classes, fv2dialogs;                                 { Standard GFV units }
...
30Sep99 LdB
---------------------------------------------------------------------}
FUNCTION MessageBox (Const Msg: String; Params: array of const;
  AOptions: Word): Word;
...
30Sep99 LdB
---------------------------------------------------------------------}
FUNCTION MessageBoxRect (Var R: TRect; Const Msg: String; Params: array of const;
  AOptions: Word): Word;
...
FUNCTION MessageBoxRectDlg (Dlg: TDialog; Var R: TRect; Const Msg: String;
  Params: array of const; AOptions: Word): Word;
...
USES sysutils,fv2Drivers, fv2Views, fv2App, fv2RectHelper{, Resource};                    { Standard GFV units }
...
{---------------------------------------------------------------------------}
function MessageBox(const Msg: String; Params: array of const; AOptions: Word): Word;
VAR R: TRect;
BEGIN
...
function MessageBoxRectDlg(Dlg: TDialog; var R: TRect; const Msg: String;
  Params: array of const; AOptions: Word): Word;
VAR I, X, ButtonCount: Integer; S: String; Control: TView;
    ButtonList: Array[0..4] Of TView;
BEGIN
   With Dlg Do Begin
     s := Format(Msg, Params);                      { Format the message }
     Control := TStaticText.create(Dlg,R, S);         { Create static text }
     ...
     For I := 0 To 3 Do
       If (AOptions AND ($0100 SHL I) <> 0) Then Begin
         ...
         Control := TButton.create(Dlg,R, ButtonName[I],
...
BEGIN
   ...
   lMS.WriteAnsiString(S);
   Dialog.SetData(lMS);                                { Set data in dialog }
   C := DeskTop.ExecView(Dialog);                    { Execute the dialog }
   lMS.Seek(0,soFromBeginning);
   If (C <> cmCancel) Then Dialog.GetData(lMS);        { Get data from dialog }
   S := lms.ReadAnsiString;
   Freeandnil(Dialog);                             { Dispose of dialog }
   result := C;                                 { Return execute result }
END;
