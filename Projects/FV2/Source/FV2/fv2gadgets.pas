...
public
      ...
      constructor InitComma(aOwner: TGroup; var Bounds: TRect);
      constructor InitKb(aOwner: TGroup; var Bounds: TRect);
      constructor InitMb(aOwner: TGroup; var Bounds: TRect);
      PROCEDURE Update;
      PROCEDURE Draw; override;
      Function  Comma ( N : LongInt ) : String;
   END;
    PHeapView = ^THeapView deprecated 'use THeapView';                            { Heapview pointer }
...
public
      ...
      PROCEDURE Draw; override;
   END;
    PClockView = ^TClockView deprecated 'use TClockView';                          { Clockview ptr }
...
constructor THeapView.Create(aOwner:TGroup;var Bounds: TRect);
begin
...
constructor THeapView.InitComma(aOwner:TGroup;var Bounds: TRect);
begin
  inherited Create(aOwner,Bounds);
  mode:=HVComma;
  OldMem := 0;
...
constructor THeapView.InitKb(aOwner:TGroup;var Bounds: TRect);
begin
  inherited Create(aOwner,Bounds);
  mode:=HVKb;
  OldMem := 0;
...
constructor THeapView.InitMb(aOwner:TGroup;var Bounds: TRect);
begin
  inherited Create(aOwner,Bounds);
  mode:=HVMb;
  OldMem := 0;
...
{---------------------------------------------------------------------------}
constructor TClockView.Create(aOwner:TGroup;Var Bounds: TRect);
BEGIN
...
VAR Sec: Word;
  ActTime: TDateTime;
BEGIN
   ActTime:= Now;  { Get current time }
   sec := trunc(ActTime * 86400) mod 3600;
   If (Abs(Sec - LastTime) >= Refresh) Then Begin     { Refresh time elapsed }
     LastTime := sec;          { Hold second }
     TimeStr := TimeToStr(ActTime);                  { Create time string }
     DrawView;                                        { Now redraw }
   End;
END;
