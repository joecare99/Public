...
USES
   ...
   video, {* from RTL-Console}
   fv2sysmsg,
   FV2Common;                                 { GFV standard units }
...
{$endif not ENDIAN_BIG}
        ...
        evMessage: (                                  { ** MESSAGE EVENT ** }
          ...
          Case Sw_Word Of
            0: (InfoPtr: Tobject);                    { Message pointer }
            1: (InfoStr: string);                    { Message variant }
          )
   END;
   PEvent = ^TEvent;
...
Function GetDosTicks:longint; deprecated 'use SysUtils.GetTickCount64';  { returns ticks at 18.2 Hz, just like DOS }
...
25May96 LdB
---------------------------------------------------------------------}
PROCEDURE MoveStr (out Dest; Const Str: String; Attr: Byte; Offset:word); overload;
PROCEDURE MoveStr (out Dest; Const Str: String; Attr: Byte); overload;
...
25May96 LdB
---------------------------------------------------------------------}
PROCEDURE MoveCStr (out Dest; Const Str: String; Attrs: Word; Offset:word); overload;
PROCEDURE MoveCStr (out Dest; Const Str: String; Attrs: Word ); overload;

{-MoveBuf------------------------------------------------------------
Count bytes are moved from Source into the low bytes of corresponding
Sw_Words in Dest. The high bytes of the Words in Dest are set to Attr,
or remain unchanged if Attr is zero.
25May96 LdB
---------------------------------------------------------------------}
PROCEDURE MoveBuf (out Dest;const Source; Attr: Byte; Count: Sw_Word);
...
25May96 LdB
---------------------------------------------------------------------}
PROCEDURE MoveChar (out Dest; C: Char; Attr: Byte; Count: Sw_Word; offset: Sw_Word);  overload;
PROCEDURE MoveChar (out Dest; C: Char; Attr: Byte; Count: Sw_Word); overload;
...
18Feb99 LdB
---------------------------------------------------------------------}
PROCEDURE FormatStr (Var Result: String; CONST Format: String;Params:array of const); deprecated 'use sysutils.Format';
...
{---------------------------------------------------------------------------}
CONST
   CheckSnow    : Boolean = False deprecated;         { Compatability only }
   MouseEvents  : Boolean = False;                    { Mouse event state }
   MouseReverse : Boolean = False;                    { Mouse reversed }
   HiResScreen  : Boolean = False deprecated;         { Compatability only }
   CtrlBreakHit : Boolean = False deprecated;         { Compatability only }
   SaveCtrlBreak: Boolean = False deprecated;         { Compatability only }
   SysErrActive : Boolean = False deprecated;         { Compatability only }
   FailSysErrors: Boolean = False deprecated;         { Compatability only }
   ButtonCount  : Byte = 0;                           { Mouse button count }
   DoubleDelay  : Sw_Word = 8*55;                           { Double click delay }
   RepeatDelay  : Sw_Word = 8*55;                           { Auto mouse delay }
   SysColorAttr : Word = $4E4F;                       { System colour attr }
   SysMonoAttr  : Word = $7070;                       { System mono attr }
   StartupMode  : Word = $FFFF deprecated;            { Compatability only }
   CursorLines  : Word = $FFFF deprecated;            { Compatability only }
   ScreenBuffer : Pointer = Nil deprecated;            { Compatability only }
   SaveInt09    : Pointer = Nil deprecated;           { Compatability only }
   SysErrorFunc : TSysErrorFunc = {$ifdef FPC_OBJFPC}@{$endif}SystemError; { System error ptr }
...
VAR
   ...
   DownTicks  : int64;                                 { Down key tick count }
   AutoTicks  : int64;                                 { Held key tick count }
   //LastWhere : Sw_Word;                                 { Last x position }
   //LastWhere : Sw_Word;                                 { Last y position }
...
{$IFDEF OS_WINDOWS}
  begin
     GetDosTicks:=GetTickCount64 div 55;
  end;
{$ENDIF OS_WINDOWS}
{$IFDEF OS_DOS}
  begin
...
begin
  ...
  if video.ScreenWidth = 0 then
    begin
      GetVideoDriver (Driver{%H-});
      if Assigned(Driver.InitDriver) then
        driver.InitDriver;
    end;
  GetVideoMode(CurrMode{%H-});
  ScreenMode:=CurrMode;
end;
...
{---------------------------------------------------------------------------}
procedure MoveStr(out Dest; const Str: String; Attr: Byte; Offset: word);
VAR I: Word; P: PWord;
BEGIN
   For I := 1 To Length(Str) Do Begin                 { For each character }
     P := @TWordArray(Dest)[I-1+Offset];                     { Pointer to Sw_Word }
     If (Attr <> 0) Then WordRec(P^).Hi := Attr;      { Copy attribute }
     WordRec(P^).Lo := Byte(Str[I]);                  { Copy string char }
   End;
END;

procedure MoveStr(out Dest; const Str: String; Attr: Byte);inline;
begin
  MoveStr(Dest,Str,Attr,0);
end;
...
{---------------------------------------------------------------------------}
procedure MoveCStr(out Dest; const Str: String; Attrs: Word; Offset: word);
VAR B: Byte; I, J: Sw_Word; P: PWord;
BEGIN
   J := 0;                                            { Start position }
   For I := 1 To Length(Str) Do Begin                 { For each character }
     If (Str[I] <> '~') Then Begin                    { Not tilde character }
       P := @TWordArray(Dest)[J+Offset];                     { Pointer to Sw_Word }
       If (Lo(Attrs) <> 0) Then
...
procedure MoveCStr(out Dest; const Str: String; Attrs: Word);inline;
begin
  MoveCStr(Dest,Str,Attrs,0);
end;
...
BEGIN
   For I := 1 To Count Do Begin
     P := @TWordArray(Dest)[offset+I-1];                     { Pointer to Sw_Word }
     If (Attr <> 0) Then WordRec(P^).Hi := Attr;      { Copy attribute }
     If (Ord(C) <> 0) Then WordRec(P^).Lo := Byte(C); { Copy character }
   End;
END;

procedure MoveChar(out Dest; C: Char; Attr: Byte; Count: Sw_Word);inline;
begin
  Movechar(dest,c,attr,count,0);
end;
...
var
  e : Mouse.TMouseEvent;
begin
  if Mouse.PollMouseEvent(e{%H-}) then
   begin
       ...
       MouseActionDown :
         begin
           ...
           if (DownButtons=e.Buttons) and (LastWhere.X=MouseWhere.X) and (LastWhere.Y=MouseWhere.Y) and
              (GetTickCount64-DownTicks<=DoubleDelay) then
             Event.Double:=true;
           DownButtons:=e.Buttons;
           DownWhere.X:=MouseWhere.x;
           DownWhere.Y:=MouseWhere.y;
           DownTicks:=GetTickCount64;
           AutoTicks:=GetTickCount64;
           if AutoTicks=0 then
             AutoTicks:=1;
  ...
  else if (AutoTicks <> 0) and (GetTickCount64 >= AutoTicks + AutoDelay) then
   begin
...
begin
  if PollSystemEvent(SysEvent) then
    begin
      fv2SysMsg.GetSystemEvent(SysEvent);
...
procedure InitSysError;
BEGIN
   SysErrActive{%H-} := True;                              { Set active flag }
END;
...
procedure DoneSysError;
BEGIN
   SysErrActive{%H-} := False;                             { Clear active flag }
END;
...
{---------------------------------------------------------------------------}
procedure FormatStr(var Result: String; const Format: String; Params:array of const);
     ...
     While (FormatIndex <= Length(Format)) Do begin   { While length valid }
       ...
       (Format[FormatIndex] = '%') Then begin         { '%' char found }
         ...
         If (FormatIndex <= Length(Format)) Then begin{ Length still valid }
           ...
           'c': S := Params[I].VChar;  { Character parameter }
             'd': S := IntToStr(Params[I].VInteger);                                   { Decimal parameter }
             's': S := Params[I].VString^;{ String parameter }
             'x': S := IntToHex(Params[I].VInteger,8);                                   { Hex parameter }
           end;
           Inc(FormatIndex);                          { Next character }
