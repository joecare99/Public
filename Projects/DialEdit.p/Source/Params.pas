...
function ExePath({%H-}ExeName: PathStr): PathStr;
...
const
    ...
    {$IFDEF FPC}
    AttrMask: cardinal = faAnyFile-faDirectory-faVolumeID;    {file types to find}
    {$ELSE}
    ...
    for ParNo:= 1 to ParamCount do begin
        ...
        if ParStr[1] = '/' then ParseOpts(ParStr)
            ...
            then begin
               ...
               {$IFDEF FPC}  ffResult := {$ENDIF} FindFirst(Path, AttrMask, SRec);
                while {$IFDEF FPC} ffResult {$ELSE} DosError {$ENDIF} = 0 do begin
                    inc(FileNo);    {count all files}
                    inc(EFiles);    {count exanded files for each ParStr}
                    PDoFile(Dir + Srec.Name, true);
                    {$IFDEF FPC}  ffResult := {$ENDIF} FindNext(SRec);
                end;
...
begin
    ...
    {$IFDEF FPC}
    ExePath:= ExpandFileName(ExeFileName);
    {$ELSE}
