...
type
    TInputLine = object(TView)
        ...
        constructor Init(const Bounds: TRect;
            AMaxLen: Sw_Integer {$ifdef FPC_OBJFPC} = 256{$endif});
...
type
    TMultiCheckBoxes = object(TCluster)
        ...
        constructor Init(var Bounds: TRect; AStrings: PSItem; ASelRange: byte;
...
type
    ...
    TDialog = object(TWindow)
      ...
      { Creates and inserts into the dialog a new TButton with the
        ...
        function NewLabel(X, Y: Sw_Integer; AText: string; ALink: PView): PLabel;
...
{ TDialog.NewLabel                                                           }
{****************************************************************************}
function TDialog.NewLabel(X, Y: Sw_Integer; AText: string; ALink: PView): PLabel;
var
    P: PLabel;
    R: TRect;
begin
...
const
    P: string{$ifopt H-}[Length(CInputLine)]{$endif} =  { Always normal string }
       CInputLine;
begin
      ...
      begin
        if (Validator = nil) or (Validator^.Transfer(Data^, @Rec, vtGetData) = 0) then
          begin                                            { No validator/data }
            ...
            b := Length(Data^);
            Move(b,  Rec, 1 );
            Move(Data^[1], (@Rec+1)^, Length(Data^) );           { Transfer our data }
          end;
      end
    else
        FillChar(Rec, DataSize, #0);                     { Clear the data area }
...
begin
    if (Data <> nil) then                              { Data ptr valid }
      begin
        if (Validator = nil)                           { No validator/data } 
           or (Validator^.Transfer(Data^, @Rec, vtSetData) = 0) then
            
          begin
            Move(Rec, b, 1);  
            setlength(Data^, b);                     { Set our data }
            Move(Rec, Data^[1], b);                  { Set our data }
          end;
      end;
...
{---------------------------------------------------------------------------}
constructor TListBox.Init(var Bounds: TRect; ANumCols: Sw_Word; AScrollBar: PScrollBar);
begin
...
begin
    if (List <> nil) and (List^.Count > 0) 
       and ((Item < List^.Count) and (Item > -1)) then
      begin
...
{****************************************************************************}
constructor TCommandCheckBoxes.Init(var Bounds: TRect; ACommandStrings: PCommandSItem);
var
    StartSItem, S: PSItem;
...
{ TCommandIcon.Init                                                          }
{****************************************************************************}
constructor TCommandIcon.Init(var Bounds: TRect; AText: string; ACommand: word);
begin
...
{ TEditListBox.Init                                                          }
{****************************************************************************}
constructor TEditListBox.Init(Bounds: TRect; ANumCols: word; AVScrollBar: PScrollBar);
...
{ TListDlg.Init                                                              }
{****************************************************************************}
constructor TListDlg.Init(ATitle: TTitleStr; Items: string; AButtons: word;
    AListBox: PListBox; AEditCommand, ANewCommand: word);
var
    Bounds: TRect;
    b: byte;
    ...
    if HasButtons then
      begin  { do buttons }
        ...
        if (AButtons and ldDelete) = ldDelete then
          begin
            Insert(NewButton(Size.X - 12, Line, 10, 2, '~D~elete', cmDelete,
                hcDelete, bfNormal));
            Inc(Line, Gap);
          end;
        Insert(NewButton(Size.X - 12, Line, 10, 2, 'O~k~', cmOK, hcOk,
            bfDefault or bfNormal));
        Inc(Line, Gap);
        Insert(NewButton(Size.X - 12, Line, 10, 2, 'Cancel', cmCancel, hcCancel, bfNormal));
        if (AButtons and ldHelp) = ldHelp then
          begin
            Inc(Line, Gap);
            Insert(NewButton(Size.X - 12, Line, 10, 2, '~H~elp', cmHelp,
