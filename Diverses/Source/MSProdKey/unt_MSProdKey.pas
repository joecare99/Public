...
const crctable (* Table for ISO/IEC 8802-3:1996 aka POSIX 1003.2 cksum *)
:array [0..255] of LongWord  = (
	$00000000,
  ...
  BINARY_LENGTH	=	15;
type TBinKey=array[0..BINARY_LENGTH] of byte;
...
Procedure 	VerifyCrc(const binkey:TBinkey);
Procedure Main;
...
begin
  ...
  pp := Pos('N',key);
  if pp<>0 then  begin
      key := alphabet[pp-1]+copy(key,1,pp-1)+copy(key,pp+1);
  end;
  ...
  for i := 1 to 25 do begin
  	 ...
  	 *)
  	for j := 0 to BINARY_LENGTH do begin
...
function TranslateAlpha(ch: Char): byte;
const  cTrans='BCDFGHJKMPQRTVWXY2346789';
var
  pp: SizeInt;
begin
  pp := pos(ch,cTrans);
...
begin
  c := (((binkey[5] shl 24)
      or (binkey[4] shl 16)
      or (binkey[3] shl 8)
      or  binkey[2])shr 4) and $3fffffff;

  writeln(format('Composite: %03u-%u\n', [c div 1000000, c mod 1000000]));
end;

procedure VerifyCrc(const binkey: TBinkey);
  var 	theirs, mine:word;
	clamped:TBinKey;
begin
  ...
  theirs := ((((binkey[14] shl 16)
      or (binkey[13] shl 8)
      or (binkey[12]))) shr 7) and $3ff;
  mine := crc(clamped, sizeof(clamped)) and $3ff;

  if (mine <> theirs) then
  	writeln(format('CRC mismatch (calculated: %u, in key: %u)',
  	    [mine, theirs]));
