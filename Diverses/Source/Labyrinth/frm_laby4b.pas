...
begin
    with pbxPreview do
      begin
        ...
        if mark then
            Canvas.Brush.Color :=
                rgb(63 + (FHLaby[x, y] * 192) div (Clen * 2 + 5), 0 + (FHLaby[x, y] * 192) div
                (Clen * 2 + 5), (FHLaby[x, y] * 192) div (Clen * 2 + 5))
        else
            Canvas.Brush.Color :=
                rgb((FHLaby[x, y] * 196) div (Clen * 2 + 5), (FHLaby[x, y] * 196) div
                (Clen * 2 + 5), (FHLaby[x, y] * 255) div (Clen * 2 + 5));
        ...
        Canvas.FillRect(Rect(xx * c * 2, yy * c * 2, (xx + 1) * c * 2, (yy + 1) * c * 2));
    ...
    for z := 0 to (Clen * 2 + 4) do
        for x := Clen - 1 downto 0 do
            for y := Clen - 1 downto 0 do
                if (FHLaby[x, y] >= z) and ((x = 0) or (y = 0) or (z > FHLaby.baselevel(x, y) - 6)) then
