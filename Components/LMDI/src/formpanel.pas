...
type
  ...
  published
    ...
    property TitleBar:TTitleBar read FTitleBar;
  end;
...
begin
  ...
  FTitleBar.Parent := TWinControl(TopPanel);
  FTitleBar.OnChangeStatus := TitleBarChangeStatus;
...
begin
  ...
  begin
    ...
    // If has clicked inside FTitleBar
    If MouseIsInsideControl(FTitleBar) = True then
    begin
    ...
    end else // click out of FTitleBar? then start Resizing (if not Maximized)
...
begin
  ...
  begin
    ...
    begin
      MIN_WIDTH := 150;
      MIN_HEIGHT := FTitleBar.Height + MG + MG;
      with WireFrame do
      begin
        Case pnDir of
          pdTopLeft:
