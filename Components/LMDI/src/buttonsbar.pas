...
begin
  if FLastWasLeftDown then
      self.MouseUp(mbLeft,[],0,0);
  Inherited DblClick;
...
begin
  ...
  FLastWasLeftDown := (ssleft in Shift) or (ssright in Shift);
  If ((FSendMouseEvents = True) and (FControlToGetMouseEvents <> nil)) then
  begin
...
begin
  if FLastWasLeftDown and not (ssLeft in Shift) then
    begin
      self.MouseUp(mbLeft,shift,x,y);
    end;
...
begin
  ...
  FLastWasLeftDown := ssleft in Shift;
  If ((FSendMouseEvents = True) and (FControlToGetMouseEvents <> nil)) then
  begin
    X := ClientOrigin.X - FControlToGetMouseEvents.ClientOrigin.X + X;
    Y := ClientOrigin.Y - FControlToGetMouseEvents.ClientOrigin.Y + Y;
