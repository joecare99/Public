...
uses SysUtils,Classes{$IFDEF FPC},singleinstance{$EndIF};

Type
  TStringArray = Array of string;
  TExceptionEvent = Procedure (Sender : TObject; E : Exception) Of Object;
 {$IFDEF FPC} TEventLogTypes = Set of TEventType; {$EndIF}
  ...
  Public
    ...
    Function GetOptionValues(Const C: Char; Const S : String) : TStringArray; overload;
...
try
sl.Text:= windows.GetEnvironmentStrings;
  count:=sl.Count;
  if count>0 then
    for j:=1 to count  do
     begin
