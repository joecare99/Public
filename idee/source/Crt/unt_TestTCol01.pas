...
procedure Execute2(wait :boolean=true) ;
...
var
    ...
    onFlushScreen:Procedure(Sender:Tobject) of object;
...
begin
    ...
    g := abs(c1.g - c2.g);
    b := abs(c1.b - c2.b);
    Result := 1.0-(r + b + g + max(r, max(b, g))) / 1024;
end;
...
begin
    Result := (c.r and $F8) shr 3 or (word(c.g) and $F8) shl 2 or (word(c.b) and $F8) shl 7;
end;
...
begin
    Result.r :=
        (i and $4) shr $2 * lComp_2 +
        (i and $8) shr $3 * lComp_1 +
        (j and $4) shr $2 * lComp2 +
        (J and $8) shr $3 * lComp1;
    Result.g :=
        (i and $2) shr $1 * lComp_2 +
        (i and $8) shr $3 * lComp_1 +
        (j and $2) shr $1 * lComp2 +
        (J and $8) shr $3 * lComp1;
    Result.b :=
        (i and $1) shr $0 * lComp_2 +
        (i and $8) shr $3 * lComp_1 +
        (j and $1) shr $0 * lComp2 +
        (J and $8) shr $3 * lComp1;
    result.a := 0;
end;

procedure MySinCos(const d:double;out s,c:Double);
      ...
      //  setlength(chh, 80 * 50);
        //          For I := 0 To maxanim Do
          begin
            ...
            FlushScreen(sb);
            if assigned(onFlushScreen) then
              onFlushScreen(nil);
            delay(10);
          end;
        FreeAndNil(bmp);
