...
begin
  ...
  FOutputOverride:=AValue;
end;

procedure TLazBuildApplication.SetUnitOutputOverride(AValue: String);
begin
  if FUnitOutputOverride=AValue then Exit;
  FUnitOutputOverride:=AValue;
end;
//End
...
//Change: JC99 
  ...
  if (Length(UnitOutputOverride) <> 0) then
    APackage.CompilerOptions.UnitOutputDirectory:=UnitOutputOverride;
//End
...
//Change: JC99 
    ...
    if (Length(UnitOutputOverride) <> 0) then
      Project1.CompilerOptions.UnitOutputDirectory:=UnitOutputOverride;
//End
...
//Change: JC99  
    ...
    if HasOption('o','output') then
      OutputOverride := GetOptionValue('o','output');

    // Unit-Output-Path
    if HasOption('FU') then
      UnitOutputOverride := GetOptionValue('FU')
    else if HasOption('unit-output') then
      UnitOutputOverride := GetOptionValue('unit-output');
//End
...
begin
  ...
  writeln('');
//Change: JC99 
  writeln('--output=<project/ide target-filename>');
  writeln('or -o <project/ide target-filename>');
  w(space+lisTargetFilenameOfProject);
  writeln('');
  writeln('--unit-output=<unit-output-directory>');
  writeln('or --FU=<unit-output-directory>');
  w(space+lisProjectOutDir);
  writeln('');
//End
