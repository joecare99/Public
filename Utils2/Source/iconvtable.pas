...
uses
  Classes, SysUtils, {$IFDEF MSWINDOWS} Windows,  {$ENDIF} {$IFDEF UNIX} Unix,  {$ENDIF}LazUTF8, FileUtil, LazUTF8Classes;
...
begin
  ...
  for i:=0 to 255 do begin
    ...
    {$IFDEF MSWINDOWS}
 if ExecuteProcess('iconv','-f '+FromEncoding+' -t '+ToEncoding+' '+Filename1+' >'+Filename2)=0
    then
 {$ENDIF}
 {$IFDEF UNIX}
