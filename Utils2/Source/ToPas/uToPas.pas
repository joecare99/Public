...
{$IFDEF t_num}
    (n:''; t:poErr{%H-}), //t_num,  //unclassified preprocessor number (not yet implemented)
{$ENDIF}
...
begin
  ...
  stEnumMember: if fDebugEnums then {%H-}WriteConstSym;
  stVar:        WriteVarSym;
  stProc:       WriteProcSym;
...
*)
  if (CurDef = 'E') or (Copy(CurDef, 1, 3) = '"E:') then begin
    if fDebugEnums then //debug?
      {%H-}WriteLn('//' + sym.UniqueName + ' in ' + CurDef);  //skip
  end else begin
    ConstSection;
