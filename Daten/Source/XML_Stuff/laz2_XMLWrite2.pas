...
type
  TXMLWriterFlag2 = (
    ...
    xwfNoIndent,xwfSpIndent,
    xwfPreserveWhiteSpace
    );
  TXMLWriterFlags2 = set of TXMLWriterFlag2;

procedure WriteXMLFile(doc: TXMLDocument; const AFileName: String; Flags: TXMLWriterFlags2 = []); overload;
procedure WriteXMLFile(doc: TXMLDocument; var AFile: Text; Flags: TXMLWriterFlags2 = []); overload;
procedure WriteXMLFile(doc: TXMLDocument; AStream: TStream; Flags: TXMLWriterFlags2 = []); overload;

procedure WriteXML(Element: TDOMNode; const AFileName: String; Flags: TXMLWriterFlags2 = []); overload;
procedure WriteXML(Element: TDOMNode; var AFile: Text; Flags: TXMLWriterFlags2 = []); overload;
procedure WriteXML(Element: TDOMNode; AStream: TStream; Flags: TXMLWriterFlags2 = []); overload;
...
type
  TXMLWriter2 = class;
  TSpecialCharCallback = procedure(Sender: TXMLWriter2; const s: DOMString;
    var idx: Integer);
  ...
  TXMLWriter2 = class(TObject)
  private
    FInsideTextNode: Boolean;
    FCanonical: Boolean;
    FIndent: DOMString; // String to use for Indentation
    FActIndent: DOMString; // Actual IndentationString
    FIndentCount: Integer;
    FBuffer: PChar;
    FBufPos: PChar;
    FCapacity: Integer;
    FLineBreak: DOMString; // Indentation
    FLineBreakEnd: DOMString; // Indentation End
    FNSHelper: TNSSupport;
    FAttrFixups: TFPList;
    FScratch: TFPList;
    FNSDefs: TFPList;
    FWriteFlags: TXMLWriterFlags2;
  ...
  public
    ...
    property WriteFlags: TXMLWriterFlags2 read FWriteFlags write FWriteFlags;
  end;

  TTextXMLWriter = Class(TXMLWriter2)
  Private
    F : ^Text;
  Protected
  ...
  TStreamXMLWriter = Class(TXMLWriter2)
  Private
    F : TStream;
  Protected
    Procedure Write(Const Buffer; Count : Longint);override;
  Public
    constructor Create(AStream: TStream;aWFlags:TXMLWriterFlags2=[]);
  end;
...
constructor TStreamXMLWriter.Create(AStream: TStream; aWFlags: TXMLWriterFlags2
  );
begin
  inherited Create;
  F := AStream;
  WriteFlags:=aWFlags;
  If xwfSpIndent in aWFlags then
    begin
      FLineBreak:=lineending;
      FIndent:='  ';
      FLineBreakEnd:='';
    end;
end;
...
constructor TXMLWriter2.Create;
var
  I: Integer;
begin
  inherited Create;
...
//  SetLength(FActIndent, 100);
  FActIndent := FLineBreak;
  FLineBreakEnd:='';
  FIndentCount := 0;
...
destructor TXMLWriter2.Destroy;
var
  I: Integer;
begin
...
procedure TXMLWriter2.wrtChars(Src: DOMPChar; Length: Integer);
var
  pb: PChar;
  wc: Cardinal;
  SrcEnd: DOMPChar;
...
procedure TXMLWriter2.wrtStr(const ws: DOMString); { inline }
begin
  wrtChars(DOMPChar(ws), Length(ws));
end;

{ No checks here - buffer always has 32 extra bytes }
procedure TXMLWriter2.wrtChr(c: DOMChar); { inline }
begin
  FBufPos^ := char(ord(c));
  Inc(FBufPos);
end;

procedure TXMLWriter2.wrtIndent; { inline }
begin
  if (xwfNoIndent in FWriteFlags) or (FIndentCount>4 ) then exit;
  wrtChars(DOMPChar(FActIndent), Length(FActIndent));
end;

procedure TXMLWriter2.IncIndent;
var
  I, NewLen, OldLen: Integer;
begin
  inc(FIndentCount);
  if xwfNoIndent in FWriteFlags then exit;
  FActIndent := FLineBreak;
  for i := 0 to FIndentCount-1 do
    FActIndent += FIndent;
  FActIndent += FLineBreakEnd;
end;

procedure TXMLWriter2.DecIndent; { inline }
var
  i: Integer;
begin
...
procedure AttrSpecialCharCallback(Sender: TXMLWriter2; const s: DOMString;
  var idx: Integer);
begin
  case s[idx] of
...
procedure TextnodeNormalCallback(Sender: TXMLWriter2; const s: DOMString;
  var idx: Integer);
begin
  case s[idx] of
    '<': Sender.wrtStr(ltStr);
...
procedure TextnodeCanonicalCallback(Sender: TXMLWriter2; const s: DOMString;
  var idx: Integer);
begin
  case s[idx] of
    '<': Sender.wrtStr(ltStr);
...
procedure CDSectSpecialCharCallback(Sender: TXMLWriter2; const s: DOMString;
  var idx: Integer);
begin
...
procedure TXMLWriter2.wrtQuotedLiteral(const ws: DOMString);
var
  Quote: DOMChar;
begin
...
procedure TXMLWriter2.WriteNode(node: TDOMNode);
begin
...
procedure TXMLWriter2.WriteNSDef(B: TBinding);
begin
  wrtChars(' xmlns', 6);
...
procedure TXMLWriter2.NamespaceFixup(Element: TDOMElement);
var
  B: TBinding;
  i, j: Integer;
  node: TDOMNode;
...
procedure TXMLWriter2.VisitElement(node: TDOMNode);
var
  i: Integer;
  child: TDOMNode;
...
begin
  ...
  begin
    ...
    wrtStr(TDOMElement(Node).TagName);
    wrtChr('>');
    if assigned(node.GetNextNodeSkipChildren) and not (node.LastChild.NodeType in [TEXT_NODE, CDATA_SECTION_NODE]) then
      wrtIndent;
  end;
  FNSHelper.EndElement;
end;

procedure TXMLWriter2.VisitText(node: TDOMNode);
begin
  ConvWrite(TDOMCharacterData(node).Data, TextSpecialChars, TextnodeCallbacks[FCanonical]);
end;

procedure TXMLWriter2.VisitCDATA(node: TDOMNode);
begin
  if not FInsideTextNode then
    wrtIndent;
  if FCanonical then
...
procedure TXMLWriter2.VisitEntityRef(node: TDOMNode);
begin
  wrtChr('&');
...
procedure TXMLWriter2.VisitPI(node: TDOMNode);
begin
...
procedure TXMLWriter2.VisitComment(node: TDOMNode);
begin
  if not FInsideTextNode then wrtIndent;
  wrtChars('<!--', 4);
  // TODO: How does this comply with c14n??
...
procedure TXMLWriter2.VisitDocument(node: TDOMNode);
var
  child: TDOMNode;
begin
  wrtStr('<?xml version="');
...
procedure TXMLWriter2.VisitDocument_Canonical(Node: TDOMNode);
var
  child, root: TDOMNode;
begin
...
procedure TXMLWriter2.VisitAttribute(Node: TDOMNode);
var
  Child: TDOMNode;
begin
  wrtChr(' ');
...
procedure TXMLWriter2.VisitDocumentType(Node: TDOMNode);
begin
  wrtStr(FLineBreak);
  wrtStr('<!DOCTYPE ');
  wrtStr(Node.NodeName);
...
procedure TXMLWriter2.VisitFragment(Node: TDOMNode);
var
  Child: TDOMNode;
begin
...
procedure WriteXMLFile(doc: TXMLDocument; AStream: TStream;
  Flags: TXMLWriterFlags2 = []);
begin
  with TStreamXMLWriter.Create(AStream,Flags) do
  try
    WriteNode(doc);
  finally
    Free;
...
procedure WriteXML(Element: TDOMNode; const AFileName: String; Flags: TXMLWriterFlags2 = []);
begin
  WriteXMLFile(TXMLDocument(Element), AFileName, Flags);
end;

procedure WriteXML(Element: TDOMNode; var AFile: Text; Flags: TXMLWriterFlags2 = []);
begin
  WriteXMLFile(TXMLDocument(Element), AFile, Flags);
end;

procedure WriteXML(Element: TDOMNode; AStream: TStream; Flags: TXMLWriterFlags2 = []);
begin
